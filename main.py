...
app = FastAPI()
# Redis client object that connects to a Redis server running on the local host
# (host="0.0.0.0") and listening on
# port 6379 (port=6379)
redis_client = redis.StrictRedis(
    host="0.0.0.0",
    port=6379,
    db=0,  # The db parameter specifies the Redis database to use. Redis supports multiple databases, and in this code we're going to use database 0, which is the default database.
    decode_responses=True,  # the responses to be decoded as strings (instead of bytes).
)
...
def add_item(item_name: str, quantity: int) -> dict[str, ItemPayload]:
    ...
    if item_id_str is not None:
        # case 1: item is already in the db
        item_id = int(item_id_str)
        # hincrby method from Redis is to increment the value of the "quantity" field by the given amount in the request
        redis_client.hincrby(f"item_id:{item_id}", "quantity", quantity)

    # otherwise, create a new item
    else:
        # When this incr call is run for the first time with the item_ids key,
        # Redis creates the key and maps it to the value 1.
        # Then, each subsequent time it's run, it increments the stored value by 1.
        item_id: int = redis_client.incr("item_ids")

        # add the item to the Redis hash
        # using the hset method and by providing a mapping for the fields (item_id, item_name, and quantity)
        redis_client.hset(
            f"item_id:{item_id}",
            mapping={
                "item_id": item_id,
...
def list_items() -> dict[str, list[ItemPayload]]:
    ...
    for name, id_str in stored_items.items():
        ...
        items.append(
            ItemPayload(
                item_id=item_id, item_name=item_name, quantity=item_quantity
            )
        )
